...
vec_parameter=np.zeros((1,3))

if id_item==0:
    vec_parameter = [r"UMFPACK",r"schur_tol_1em16_M_umf",r"schur_tol_1em10_M_umf"]
elif id_item==1:
    vec_parameter = [r"UMFPACK",r"M_tol_1em16_Schur_1em16",r"M_tol_1em10_Schur_1em16"]

vec_file_name=["Schur_var_M_umf","M_var_Schur_1em16"]
...
for id_var in range(id_var_start, id_var_end):
    ...
    data_raw = [line.split() for line in open('data_error_mm_'+vec_parameter[id_parameter]+'.txt','r')]
    ...
    for i in range(n_row):
        if id_var==0:
            data_ndofs[i]=2**(i+1)*vec_deg[id_var]
    ...
    text_offset_x_alter = dof_ref[11]
    ...
    for id_parameter in range(id_parameter_start, id_parameter_end):
        plt.loglog(data_ndofs[vec_error_plot_start[id_parameter]:], data_error[vec_error_plot_start[id_parameter]:,id_parameter],'ks', markerfacecolor=vec_markerfacecolor[id_parameter],markeredgecolor=vec_markeredgecolor[id_parameter], color=vec_markeredgecolor[id_parameter],linewidth=1.0,label=vec_legend[id_parameter])    
    ...
    f.savefig("py_error_comparison_solution_strategy_"+vec_FEM[id_FEM]+"_"+vec_file_name[id_item]+"_"+vec_var[id_var]+".pdf", bbox_inches='tight')    
