...
hold on;
UMF_bound=6E-20*n_dofs.^1.0;
loglog(n_dofs, UMF_bound,'--k');
text(0.5*n_dofs(1),1*UMF_bound(12),'6\times10^{-20}\timesN^{1.0}','HorizontalAlignment','left','Color','k','FontSize',14);

% CG_bound=6E-19*n_dofs.^2;
% loglog(n_dofs, CG_bound,'--k');
% text(0.5*n_dofs(8),1*CG_bound(6),'6\times10^{-19}\timesN^2','HorizontalAlignment','left','Color','k','FontSize',14);


set(h,'Units','Inches');
pos = get(h,'Position');
set(h,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
print(h,'Pois_MM_UMFvsCG_solu','-dpdf','-r0');