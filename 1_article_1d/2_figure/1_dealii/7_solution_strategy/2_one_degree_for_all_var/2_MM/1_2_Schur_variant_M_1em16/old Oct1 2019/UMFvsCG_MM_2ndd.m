...
hold on;
% UMF_bound=2E-16*n_dofs.^1.0;
% loglog(n_dofs, UMF_bound,'--k');
% text(0.5*n_dofs(5),1*UMF_bound(2),'2\times10^{-16}\timesN^{1.0}','HorizontalAlignment','left','Color','k','FontSize',14);

CG_bound=5E-16*n_dofs.^2;
loglog(n_dofs, CG_bound,'--k');
text(0.15*n_dofs(1),1*CG_bound(6),'5\times10^{-16}\timesN^{2.0}','HorizontalAlignment','left','Color','k','FontSize',14);

set(h,'Units','Inches');
pos = get(h,'Position');
set(h,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
print(h,'Pois_MM_UMFvsCG_2ndd','-dpdf','-r0');