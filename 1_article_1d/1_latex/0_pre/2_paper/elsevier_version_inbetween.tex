...
For larger $p$, since $E_{\rm T}$ decreases faster, smaller ${E}_{\text{min}}$ can be obtained, see Fig.~\ref{E_min_benchmark_Poisson} for the statistics. It also shows that $E_{\rm min}$ tends to deteriorate by differentiation since $E_{\rm T}$ decreases slower and $\alpha_{\rm R}$ increases slightly. In general, smaller $E_{min}$ can be obtained using the mixed FEM compared to using the standard FEM.
...
\begin{table}[!ht]
\small
\captionof{table}{$\alpha_{\rm R}$ in terms of the product of a constant and an unknown for the one-dimensional second order Poisson equations.}
\hspace{3.5cm}
...
\begin{table}[!ht]
\small
\captionof{table}{$\alpha_{\rm R}$ in terms of the product of a constant and an unknown for the one-dimensional second order differential equations.}
\hspace{3.5cm}
\begin{subtable}{0.4\textwidth}
\caption{The standard FEM}
\begin{tabular}{l l c}
